class AdvancedPortugueseGame {
    ...
    bindEvents() {
        ...
        // Botão de revisão usando delegação de eventos
        document.addEventListener('click', (e) => {
            if (e.target.id === 'review-btn-portuguese') {
                this.showReview();
            }
        });
...
document.addEventListener('DOMContentLoaded', () => {
    try {
        ...
        if (session.age < 15) {
            alert('Este jogo é destinado para estudantes a partir de 15 anos. Você será redirecionado para a página inicial.');
            window.location.href = 'index.html';
            return;
        }
